...
import { Bars3Icon } from '@heroicons/react/24/outline'; // Add this import

const Navbar = ({ onSidebarToggle }) => { // Accept the onSidebarToggle prop
    ...
    return (
       <>
          <header className={
            !navState ? 'absolute top-0 left-0 right-0 opacity-100 z-50 blur-effect-theme' : 'fixed top-0 left-0 right-0 h-[15vh] flex items-center justify-center opacity-100 z-[200] blur-effect-theme'
          }>
            <nav className='flex items-center justify-between nike-container'>
                ...
                <ul className='flex items-center justify-center gap-2'>
                    ...
                    <li className='grid items-center'>
                        <MagnifyingGlassIcon className={`icon-style ${navState && "text-slate-900  duration-300"}`} />
                    </li>
                    <li className='grid items-center'>
                        <HeartIcon className={`icon-style ${navState && "text-slate-900  duration-300"}`} />
                    </li>
                    <li className='grid items-center'>
